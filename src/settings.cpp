	...
	if (!inConfigFile.is_open()) {
		...
		for (const auto &[key, value] : vars) {
			...
			if (value.type() == typeid(int)) {
				...
				g_utils->log(DEBUG, fmt::format("Settings, Unknown type {}",
				                                value.type().name()));
			}
		}
		outConfigFile.flush();
		...
		for (std::string line; std::getline(inConfigFile, line);) {
			...
			if (quote_pos != std::string::npos) {
				...
				if (end_quote_pos != line.size() - 1)
					throw std::runtime_error(fmt::format(
					    "Settings, Invalid config, doesn't end with end quote, "
					    "{}:{} ({})",
					    config_name, config_line, line.c_str()));
			...
			if (type == typeid(int)) {
				...
				if (isNumber == false)
					throw std::runtime_error(fmt::format(
					    "Settings, Invalid config, expected float got string, "
					    "{}:{} ({})",
					    config_name, config_line, line.c_str()));
				...
				if (isNumber == true)
					throw std::runtime_error(fmt::format(
					    "Settings, Invalid config, expected string got number, "
					    "{}:{} ({})",
					    config_name, config_line, line.c_str()));
