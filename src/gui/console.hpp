...
struct AppConsole {
	...
	AppConsole() {
		...
		ShowFilters = false;
		AddLog("Initalized console!");
		AddLog("Type help to see all commands.");
	...
	void ExecCommand(const char* command_line) {
		...
		if (params[0] == "clear") {
			...
			if (params.size() < 3) {
				...
				try {
					g_memoryEditor->memData = (void*)(g_game->getBaseAddress() +
					                                  std::stoi(params[1], nullptr, 0));
					g_memoryEditor->memSize = std::stoi(params[2], nullptr, 0);

					if (!g_memoryEditor->isOpen) g_memoryEditor->isOpen = true;
				} catch (const std::exception& e) {
					AddLog(fmt::format("[error] Invalid parameter used, exception '{}'",
					                   e.what())
					           .c_str());
				}
			}
...
class console : public gui {
 public:
	void onInit() { con = std::make_unique<AppConsole>(); }
	using gui::gui;
